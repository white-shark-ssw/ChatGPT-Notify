...
典型需要发送：代码实现或修改、Bug/日志/崩溃分析、CI/构建/IPA/测试证据分析、GitHub 实质性处理、多步骤技术调查、文件产物完成、较长项目决策分析。
...
2. 形成准备发送给用户的最终回答。
3. 从最终回答生成通知标题与正文预览。
4. 如满足通知条件，向 `white-shark-ssw/ChatGPT-Notify` PR `#1` 添加一条顶层 PR Conversation 评论。
...
通知中心 workflow 只监听 `issue_comment: created`，因此创建后立即编辑不会触发第二次推送。真实测试已经确认，Action 会保留创建事件中的原始 payload 并完成 Bark 推送。

通知中心随后会用 GitHub Actions 自带的 `GITHUB_TOKEN` 删除该临时评论。当前 workflow 同时授予 `issues: write` 与 `pull-requests: write`；run #10 已真测确认 Bark 推送与删除评论均成功。因此正常情况下 PR 不会持续积累占位评论。

如果创建通知评论成功但立即擦除失败，最终回答中必须明确注明：

`Bark 通知已触发，但 GitHub 临时通知正文擦除失败，请检查通知 PR #1。`

即时擦除与 Action 删除是两层机制：前者尽量缩短公开正文暴露时间，后者负责保持 PR 整洁。
...
这样所有 ChatGPT 项目完成通知在 Bark 中进入同一个独立分组，不会和其他 Bark 来源混在一起。
